...
class chotot_projectSpider(scrapy.Spider):
    ...
    base_url = 'https://gateway.chotot.com/v1/public/xproperty/projects/_search?region_v2='
    ...
    def parse_district(self, response):
        ...
        for region in regions:
            query_string_params = region+ '&offset=0&status=active'
            yield scrapy.Request(
                url=self.base_url + query_string_params,
                method='GET',
                headers=self.headers_project,
    ...
    def parse_project(self,response):
        ...
        for page in range(1,number_page+1):
            query_string = region + '&offset=' + str((page-1)*10) + '&status=active'
            yield scrapy.Request(
